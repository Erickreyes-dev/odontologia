...
export function DataTable<TData, TValue>({
  ...
  React.useEffect(() => {
    const timeout = window.setTimeout(() => {
      onPageChange(1, globalFilter);
    }, 300);

    return () => window.clearTimeout(timeout);
  }, [globalFilter, onPageChange]);
    ...
    onPaginationChange: (updater) => {
      const next =
        typeof updater === "function"
          ...
          : updater;
      onPageChange(next.pageIndex + 1, globalFilter);
    },
  ...
  return (
    <div className="rounded-md border p-4">
      <div className="flex flex-col md:flex-row items-center py-4 justify-between gap-2">
        ...
        <ExportExcelButton data={data as Record<string, unknown>[]} fileName="citas" />
      </div>
      ...
      <div className="flex items-center justify-between py-4">
        <Button
          variant="outline"
          size="sm"
          onClick={() => onPageChange(page - 1, globalFilter)}
          disabled={page <= 1}
        >
          Anterior
        </Button>
