...
function Actions({ row, catalogs }: { row:EgresoRow; catalogs:Catalogs }) { const [open,setOpen]=useState(false); const [pending,startTransition]=useTransition(); const [error,setError]=useState<string|null>(null); return <Dialog open={open} onOpenChange={setOpen}><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon"><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuLabel>Acciones</DropdownMenuLabel><DialogTrigger asChild><DropdownMenuItem disabled={row.esAutomatico} onSelect={(e)=>e.preventDefault()}>Editar</DropdownMenuItem></DialogTrigger><DropdownMenuItem disabled={row.esAutomatico||pending} className="text-destructive" onClick={()=>{ if(confirm("¿Eliminar este egreso?")) startTransition(async()=>{const r=await deleteEgreso(row.id); if(!r.ok) setError(r.message??"No se pudo eliminar");});}}>Eliminar</DropdownMenuItem></DropdownMenuContent></DropdownMenu><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl"><DialogHeader><DialogTitle>Editar egreso</DialogTitle></DialogHeader><EgresoForm catalogs={catalogs} initialData={row} submitLabel="Guardar" action={async(payload)=>{const r=await updateEgreso(row.id,payload); if(!r.ok) return r; setOpen(false); return {ok:true};}} />{error?<p className="text-sm text-destructive">{error}</p>:null}</DialogContent></Dialog> }
